...
import { inferSession } from "./tradingviewWebhook";
import {
  checkXauusdMarketSchedule,
  FRESHNESS_LIMITS_MS,
...
// Minimum interval between upstream provider calls per timeframe (durable source of truth)
...
let lastSourceUsed: "twelvedata" | "tradingview" | "cache" | "none" = "none";
...
/**
 * Parses Twelve Data datetime format (e.g. "2026-09-13 20:05:00") to UTC epoch milliseconds
 */
...
/**
 * Normalizes raw Twelve Data JSON array of candles into internal CandleItem[]
 */
export function normalizeTwelveDataCandles(
  ...
  // Twelve Data sends newest first -> sort ascending (chronological: oldest to newest)
...
/**
 * Reads provider synchronization metadata from Firestore
 * Document: marketData/XAUUSD/providerStatus/twelveData
 */
...
/**
 * Persists provider synchronization metadata to Firestore
 */
...
/**
 * Reads daily provider usage from Firestore
 * Document: marketData/XAUUSD/providerUsage/{YYYY-MM-DD}
 */
...
/**
 * Increments daily provider request counts in Firestore
 */
...
/**
 * Persists normalized candles and snapshots to Firestore with idempotent IDs
 */
export async function syncToFirestore(
  ...
  try {
    ...
    // 3. Rolling candles with timestamp as ID (strict idempotency, no duplicates)
...
/**
 * Loads market snapshots and rolling candle sequence from Firestore
 */
...
// ============================================================================
// HARDENED PRIMARY FETCH FUNCTION
// ============================================================================
...
export async function getXauusdMarketData(options?: {
  ...
  // If hard limit reached (750+ requests/day), do not make any upstream calls
  if (hardLimitReached) {
    console.warn(
  ...
  // Durable source of truth: providerStatus.lastFetchM5, lastFetchM15, lastFetchH1
  const checkDue = (
    ...
    if (force) {
      // Force refresh safety: must still respect 60 seconds minimum interval
    ...
    // Standard interval check: M5 (5m), M15 (15m), H1 (60m)
  ...
  if (timeframesToFetch.length === 0) {
    ...
    const checkFreshness = (tf: SupportedDeskTimeframe, ts: number | null) => {
      if (!ts) return false;
      if (!marketStatus.isOpen) return true; // Valid close when market closed
  ...
  // Pre-load existing data so non-due timeframes remain fully populated
  ...
  if (upstreamRequestsCount > 0) {
    ...
    // Persist provider status and usage
    await Promise.all([
  ...
  lastSourceUsed = upstreamRequestsCount > 0 ? "twelvedata" : "cache";
